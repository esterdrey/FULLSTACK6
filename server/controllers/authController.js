    ...
    db.query(sql, [username], (err, results) => {
        ...
        if (user.blocked) {
            return res.status(403).json({
                message: 'This account is blocked. Contact admin.'
            });
        }
        ...
        if (user.stored_password !== password) {
            ...
            if (newAttempts >= 3) {
                db.query(
                    'UPDATE users SET login_attempts = ?, blocked = 1 WHERE id = ?',
                    [newAttempts, user.id]
                );

                return res.status(403).json({
                    message: 'Account locked after 3 failed attempts. Contact admin.'
                });
            }
            ...
            return res.status(401).json({
                message: `Invalid password. ${3 - newAttempts} attempt(s) remaining.`
            });
        }
